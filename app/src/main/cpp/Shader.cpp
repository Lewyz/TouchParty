...
Shader *Shader::loadShader(
    ...
    if (program) {
        ...
        if (linkStatus != GL_TRUE) {
            ...
            if (!colorUniformName.empty()) {
                colorUniform = glGetUniformLocation(program, colorUniformName.c_str());
            }

            GLint useTextureUniform = -1;
            if (!useTextureUniformName.empty()) {
                useTextureUniform = glGetUniformLocation(program, useTextureUniformName.c_str());
            }
...
void Shader::drawIndexed(const void *vertices, size_t vertexStride, size_t positionOffset,
                         size_t uvOffset, const uint16_t *indices, size_t indexCount) const {
    const uint8_t *vData = static_cast<const uint8_t *>(vertices);
    ...
    glDrawElements(GL_TRIANGLES, static_cast<GLsizei>(indexCount), GL_UNSIGNED_SHORT, indices);
...
void Shader::setColor(float r, float g, float b, float a) const {
    if (colorUniform_ != -1) {
        glUniform4f(colorUniform_, r, g, b, a);
    }
}

void Shader::setUseTexture(bool useTexture) const {
    if (useTextureUniform_ != -1) {
        glUniform1i(useTextureUniform_, useTexture ? 1 : 0);
    }
}